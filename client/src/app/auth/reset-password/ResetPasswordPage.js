...
export default function ResetPasswordPage() {
  ...
  // Handle change for new password, also validating strength
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(strengthErrors).length > 0) {
      setPasswordErrors(prev => ({ ...prev, newPassword: strengthErrors }));
      setError('Please fix password strength issues.');
    ...
    try {
      ...
      throw new Error(response.data?.message || 'Password reset failed without specific message.');
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              name="newPassword"
              value={newPassword}
              onChange={handleNewPasswordChange} // Use the new handler
              className={`mt-1 block w-full pr-10 px-4 py-2 border ${
                passwordErrors.newPassword && Object.keys(passwordErrors.newPassword).length > 0
                  ? 'border-red-500'
                  : 'border-gray-300'
            ...
            {passwordErrors.newPassword && (
              <ul className="mt-1 text-sm text-red-600 list-disc pl-5">
                {passwordErrors.newPassword.length && <li>{passwordErrors.newPassword.length}</li>}
                {passwordErrors.newPassword.upperCase && <li>{passwordErrors.newPassword.upperCase}</li>}
                {passwordErrors.newPassword.lowerCase && <li>{passwordErrors.newPassword.lowerCase}</li>}
                {passwordErrors.newPassword.number && <li>{passwordErrors.newPassword.number}</li>}
                {passwordErrors.newPassword.specialChar && <li>{passwordErrors.newPassword.specialChar}</li>}
              </ul>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 mt-6">
            ...
            <button
              ...
              disabled={isResetting || Object.keys(passwordErrors.newPassword || {}).length > 0 || newPassword !== confirmPassword}
            >
