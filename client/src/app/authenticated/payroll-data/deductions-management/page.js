...
import {
  fetchEmployeesWithDeductions,
  updateEmployeeDeduction,
} from "@/app/components/utils/deductionsApi";
...
export default function DeductionsManagementPage() {
  ...
  const loadData = async (startDate = null, endDate = null) => {
    ...
    setError(null);
    try {
      const data = await fetchEmployeesWithDeductions(startDate, endDate);
      setEmployees(data);
      ...
      if (data.length > 0) {
        ...
        data.forEach((employee) => {
          employee.employeeDeductions?.forEach((deduction) => {
            if (deduction.deduction && deduction.deduction.deductionType) {
              deductionTypes.add(deduction.deduction.deductionType);
            }
          });
        });
        ...
        setAllDeductionTypes(Array.from(deductionTypes));
      }
      ...
      setError(
        err.message || "Failed to load deductions data. Please try again."
      );
      setLoading(false);
    }
  };
  ...
  const handleSaveDeduction = async (deductionId, updateData) => {
    setLoading(true);
    try {
      await updateEmployeeDeduction(deductionId, updateData);
      const updatedData = await fetchEmployeesWithDeductions();
  ...
  return (
    <div className="bg-gray-50 text-gray-900 px-6 py-8 min-h-screen rounded-lg shadow-md">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <PayrollSectionDropdown current="Other Deductions" />
