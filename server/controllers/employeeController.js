const { Employee, User, Company, JobTitle, Department, Project, Region } = require('../models');
const { Op } = require('sequelize');
// const softDelete = require('../utils/softDelete'); // Assuming you have this utility
...
exports.createEmployee = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to create employee', errors: error.errors ? error.errors.map(e => e.message) : [error.message] });
    }
};
...
where: whereClause,
           attributes: [
               'id', 'firstName', 'middleName', 'lastName', 'workEmail',
               'gender', 'dateOfBirth', 'staffNo', 'status', 'paymentMethod',
               'accountName', 'accountNumber', 'bankName', 'mobileNumber',
               'nationalId', 'passportNo', 'maritalStatus', 'residentialStatus',
               'jobTitleId', 'departmentId', 'companyId', 'projectId',
               'employmentType', 'basicSalary', 'reportingToId', 'createdByUserId'
           ],
           include: [
               {
                   model: User,
    ...
    try {
        ...
        const employees = await Employee.findAll({
            ...
            include: [
                ...
                { model: Employee, as: 'reportsTo', attributes: ['id', 'firstName', 'lastName'] },
            ],
        });
        res.status(200).json(employees);
    } catch (error) {
        console.error('Error fetching employees by company:', error);
        res.status(500).json({ message: 'Failed to fetch employees by company', error: error.message });
    }
};
...
exports.filterEmployees = async (req, res) => {
    try {
        ...
        const { jobTitle, department, employmentType, paymentMethod, createdBy, project } = req.query;
        ...
        res.status(500).json({ message: 'Failed to filter employees', error: error.message });
    }
};
    ...
    try {
        ...
        const updatedEmployee = await Employee.findByPk(id, {
            include: [
                ...
                { model: Department, as: 'department', attributes: ['id', 'title'] },
                { model: Project, as: 'project', attributes: ['id', 'name'] },
                { model: Employee, as: 'reportsTo', attributes: [ 'firstName', 'lastName'],   foreignKey: 'reportingToId',  targetKey: 'id' },
            ],
        });
        res.status(200).json(updatedEmployee);
    } catch (error) {
        console.error('Error updating employee:', error);
        res.status(500).json({ message: 'Failed to update employee', errors: error.errors ? error.errors.map(e => e.message) : [error.message] });
    }
};
...
exports.softDeleteEmployee = async (req, res) => {
    try {
        const { id } = req.params;
        const [updatedRows] = await Employee.update({ deletedAt: new Date() }, {
            where: { id },
            attributes: {
        ...
        res.status(500).json({ message: 'Failed to soft delete employee', errors: error
...
exports.searchEmployee = async (req, res) => {
    try {
        ...
        if (!companyId) {
            return res.status(400).json({ message: 'Company ID is required for search' });
        }
        if (!query) {
...
exports.getAllEmployeeDetails = async (req, res) => {
    try {
        const { companyId } = req.query;
        const whereClause = companyId ? { companyId, deletedAt: null } : { deletedAt: null };
        ...
        const Region = require('../models/regionModel');        const { Earnings, EmployeeEarnings } = require('../models/earningsModel');
        const { Deduction } = require('../models/deductionsModel');
        const { Allowance, EmployeeAllowance} = require('../models/allowancesModel');
        const employees = await Employee.findAll({
            where: whereClause,
            subQuery: false,
            attributes: {
        ...
        const formattedDetails = employees.map(employee => {
            const base = {
                ...
                reportingTo: employee.reportsTo ? `${employee.reportsTo.firstName} ${employee.reportsTo.lastName}` : null,
            };
            const earnings = employee.employeeEarnings ? employee.employeeEarnings.map(ee => ({
            ...
            const deductions = employee.deductions ? employee.deductions.map(deduction => {
            ...
            const allowances = employee.employeeAllowances ? employee.employeeAllowances.map(ea => ({
        ...
        res.status(500).json({ message: 'Failed to fetch all employee details', error: error.message });
    }
};
